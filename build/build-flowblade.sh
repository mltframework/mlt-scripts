...
# This script builds mlt, flowblade, and many of their dependencies.
# It can accept a configuration file, default: build-flowblade.conf
...
CONFIGFILE=build-flowblade.conf
...
function read_configuration {
  ...
  # This is for replacement in start-flowblade
...
function set_globals {
  ...
  REVISIONS[2]=""
  if test 0 = "$FLOWBLADE_HEAD" -a "$FLOWBLADE_REVISION" ; then
    REVISIONS[2]="$FLOWBLADE_REVISION"
  fi
  REVISIONS[3]=""
  ...
  #####
  # ffmpeg
  CONFIG[0]="./configure --prefix=$FINAL_INSTALL_DIR --disable-doc --enable-gpl --enable-version3 --enable-shared --enable-debug --enable-pthreads --enable-runtime-cpudetect"
...
function get_subproject {
  ...
  elif test "hg" = "$REPOTYPE" ; then
      # If the dir is there, check if it is a hg repo
      if test -d "$1" ; then
          # Change to it
          cmd cd $1 || die "Unable to change to directory $1"
          debug "About to look for hg repo"
          hg status 2>&1 | grep "Not a branch" &> /dev/null
          if test 0 != $? ; then
              # Found hg repo
              # TODO: handle REVISION
              debug "Found hg repo, will update"
              feedback_status "Pulling hg sources for $1"
              cmd hg fetch || die "Unable to hg pull sources for $1"
              cmd hg update || die "Unable to hg update sources for $1"
          else
              # A dir with the expected name, but not a hg repo, bailing out
              PWD=`pwd`
              die "Found a dir with the expected name $1 ($PWD), but it was not a hg repo. Unable to proceed. If you have old flowblade sources, please delete these directories, before rerunning the script."
          fi
      else
          # No hg repo
          debug "No hg repo, need to check out"
          feedback_status "Cloning hg sources for $1"
          cmd hg clone --quiet $REPOLOC $REVISION || die "Unable to hg branch source for $1 from $REPOLOC"
          cmd cd $1 || die "Unable to change to directory $1"
      fi
  fi # git/svn/hg
...
function configure_compile_install_subproject {
  ...
  # Special hack for flowblade
  if test "flowblade" != "$1" ; then
  ...
  if test "1" = "$NEED_SUDO" ; then
    ...
    TMPNAME=`mktemp -t build-flowblade.installoutput.XXXXXXXXX`
    # At least kdesudo does not return an error code if the program fails
    # Filter output for error, and dup it to the log
    if test "flowblade" = "$1" ; then
      $SUDO cp -a flowblade-trunk "$FINAL_INSTALL_DIR" > $TMPNAME 2>&1
      $SUDO cp -a ../mlt/src/swig/python/{_mlt.so,mlt.py} "$FINAL_INSTALL_DIR"/flowblade-trunk
    else
  ...
  elif test "flowblade" = "$1" ; then
    cmd cp -a flowblade-trunk "$FINAL_INSTALL_DIR" || die "Unable to install $1"
    cmd cp -a ../mlt/src/swig/python/{_mlt.so,mlt.py} "$FINAL_INSTALL_DIR"/flowblade-trunk
  else
...
End-of-startup-script-template
  ...
  $SUDO cp $TMPFILE $FINAL_INSTALL_DIR/start-flowblade || die "Unable to create startup script - cp failed"

  TMPFILE=`mktemp -t build-flowblade.env.XXXXXXXXX`
  log Creating environment script in $TMPFILE
  # Note that if you change any of this, you may also want to change start-flowblade
  cat > $TMPFILE <<End-of-environment-setup-template
#!/bin/sh
# Set up environment
# Source this file using a bash/sh compatible shell, to get an environment, 
# where you address the binaries and libraries used by your custom Flowblade build  
